...
/// <summary>
/// A processor that always succeeds, captures as many tokens as <paramref name="RepeatCount" /> requests, or all when
/// -1.
/// </summary>
...
internal sealed record RepeatingWildCardNodeProcessor(string CaptureName, int RepeatCount = -1) : INodeProcessor
{
    ...
    public TokenProcessingResult ProcessToken(ReadOnlyMemory<Token> tokens, CommandBuilder builder)
    {
        ...
        return repeatCount == tokens.Length
                   ...
                   : new TokenProcessingResult(true, builder, tokens[repeatCount..]);
    }
}
