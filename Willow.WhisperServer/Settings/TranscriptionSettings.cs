...
/// the accuracy of the transcription. However, this can also slow down the process due to the additional
/// calculations
/// required.
/// </item>
/// <item>
/// <b><i>Lower <paramref name="BestOf" /></i></b>: Speeds up the transcription process by reducing the number of
/// options considered, but it might lead to less accurate results if the correct option is not among those top
/// few
/// considered.
/// </item>
/// </list>
...
/// In simpler terms, <paramref name="Patience" /> is like a measure of how long the system will keep looking for
/// better
...
/// system exhaustively checks more options. However, it also means longer processing times, as the system
/// takes longer
...
/// faster. However, this might come at the cost of accuracy, as the system may not explore enough options
/// before making
/// a decision.
/// </item>
/// </list>
...
/// sequences, potentially making the transcription more concise. However, it might omit necessary details or
/// context
/// by favoring brevity.
/// </item>
/// <item>
...
/// words, which can make the transcription more diverse and less redundant. However, it might inadvertently
/// avoid
...
/// avoiding repetitions, which might be necessary in certain contexts. But, this could also lead to
/// transcriptions
...
/// the repetition of longer phrases. However, it might limit the system's ability to use common phrases or
/// expressions
...
/// could be important in certain contexts. But, it may lead to transcriptions that are repetitive, especially
/// with
...
/// potentially capturing unexpected or less common interpretations of the speech. However, this can also lead
/// to more
...
/// transcriptions, sticking closely to the most probable options. This can make the transcription less prone
/// to
...
/// The threshold for the gzip compression ratio. If the ratio exceeds this value, the sampling is considered a
/// failure.
...
/// before considering it a failure, which could be useful in capturing longer, more detailed transcriptions.
/// However,
...
/// generating text, potentially leading to more concise and relevant transcriptions. But, this might cause it
/// to miss
...
/// leading to potentially higher quality and more accurate transcriptions. However, this might cause the
/// system to
...
/// confidence, which can be beneficial in ambiguous or complex speech segments. However, this could result in
/// less
...
/// audio as silent, which can be useful to avoid skipping over quiet or unclear speech. However, this might
/// lead to
...
/// potentially leading to a cleaner transcription without unnecessary breaks. But, this may result in missing
/// out on
...
/// accurate transcription by building on what has already been transcribed. However, this might cause the
/// system to
...
/// independently, potentially leading to a transcription with less repetitive loops. But this can result in
/// losing
...
/// transcriptions before resetting. This can lead to more diverse and potentially insightful transcripts but
/// might
...
/// starts taking creative liberties, keeping the transcription closely tied to the established context.
/// However, this
...
/// cleanly, without including unnecessary silence or pauses at the beginning. It makes the transcript tidier
/// but
...
/// everything from the very start, including any silence or noise. This can provide a complete record but may
/// include
...
/// more focused transcription by leaving out unnecessary elements. However, over-suppression might result in
/// missing
...
/// comprehensive transcription, capturing a wider range of sounds and words. But this can include unwanted
/// noise or
/// irrelevant words, potentially cluttering the transcript.
/// </item>
/// </list>
/// </param>
public readonly record struct TranscriptionSettings(int BeamSize,
                                                    int BestOf,
                                                    float Patience,
                                                    float LengthPenalty,
                                                    ...
                                                    int[] SuppressTokens)
{
    private static readonly float[] _temperatureDefaults =
    [
        0.0f,
        0.2f,
        0.4f,
    ...
    public TranscriptionSettings() : this(5,
                                          5,
                                          1.0f,
                                          1.0f,
                                          1.0f,
