...
internal static class TranscriptionCommandExtensions
{
                                         ...
                                         TranscriptionSettings settings)
    {
        ...
        var temperatureList = new PyList(settings.Temperature.Select(t => new PyFloat(t)).ToArray());
        // ReSharper disable once CoVariantArrayConversion
        var suppressTokensList = new PyList(settings.SuppressTokens.Select(t => new PyInt(t)).ToArray());

        var result = model.transcribe(
            audio: io.BytesIO(audioData),
    ...
    private static string InterpretResult(dynamic result)
    {
        ...
        var strings = iterator.Cast<dynamic>()
                              .Select(x => ((string)x.text).ToString().Trim());
