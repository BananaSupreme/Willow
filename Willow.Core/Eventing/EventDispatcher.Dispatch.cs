...
internal sealed partial class EventDispatcher
{
    ...
    private async Task RunEvents<TEvent>(List<IEventHandler<TEvent>> handlers, TEvent @event)
        where TEvent : notnull
    {
        var exceptions =
    ...
    private async Task HandleEvent<TEvent>(IEventHandler<TEvent> handler, TEvent eventItem)
        where TEvent : notnull
    {
    ...
    private void AddLogContext<TEvent>(IEventHandler<TEvent> handler, TEvent _)
        where TEvent : notnull
    {
        _log.AddContext("handlerName", TypeExtensions.GetFullName(handler.GetType()));
    }

    private void LogException<TEvent>(IEventHandler<TEvent> _, TEvent __, Exception ex)
        where TEvent : notnull
    {
